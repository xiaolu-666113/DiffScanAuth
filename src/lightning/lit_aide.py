"""Lightning module for AIDE-style baseline."""
...
class LitAIDE(pl.LightningModule):
    """Train/eval wrapper for the AIDE-style detector baseline."""
    ...
    def __init__(self, model_cfg: dict[str, Any], optim_cfg: dict[str, Any]) -> None:
        ...
        self.model = AIDEStyleDetector(
            rgb_backbone_name=str(model_cfg.get("rgb_backbone_name", "convnext_tiny")),
            artifact_backbone_name=str(model_cfg.get("artifact_backbone_name", "convnext_tiny")),
    ...
    def _loss(self, logits: torch.Tensor, label: torch.Tensor) -> torch.Tensor:
        if self.pos_weight is not None:
            weight = torch.tensor([float(self.pos_weight)], device=logits.device, dtype=logits.dtype)
    ...
    def test_step(self, batch: dict[str, Any], batch_idx: int) -> None:
        ...
        for iid, ipath, y, p in zip(batch["image_id"], batch["image_path"], label.detach().cpu().tolist(), prob.detach().cpu().tolist()):
